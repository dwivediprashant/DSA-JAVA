...
public class LexicographNfrequency {
    ...
    public static void lexiOrderIfSameFreq(String[] sortWords, int[] repeatedCharInEachString) {
        for (int i = 0; i < sortWords.length; i++) {
            ...
            for (int j = i + 1; j < sortWords.length; j++) {
                if ((largest.compareTo(sortWords[j]) < 0)
                        && (repeatedCharInEachString[j] == repeatedCharInEachString[i])) {
                    largest = sortWords[j];
                    largestIdx = j;
                }
            }
    ...
    public static void main(String args[]) {
        ...
        // String[]
        // words={"strings","character","spoons","apply","programming","rotator","alloy"};
        // user input
        ...
        // now sorting if any two strings have same frequency according to
        // lexicograhical order-->compareTo function
        lexiOrderIfSameFreq(sortWords, repeatedCharInEachString);
        // printing overall result
        System.out.println(
                "Increasing order based on count of repeating characters nad if same count then lexicographical order is taken :-");
        for (int i = 0; i < sortWords.length; i++) {
